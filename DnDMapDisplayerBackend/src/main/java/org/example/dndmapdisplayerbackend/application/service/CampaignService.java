...
public class CampaignService implements CreateCampaignUseCase, GetCampaignUseCase, DeleteCampaignUseCase, UpdateCampaignUseCase, GetAllCampaignsFromUserUseCase {
    ...
    public CampaignService(CampaignRepositoryPort repository, UserRepositoryAdapter userRepositoryAdapter) {
    ...
    public Campaign createCampaign(String name, String description, String email) {
        ...
        User user = userRepositoryAdapter.findByEmail(email).orElseThrow(UserNotFoundException::new);
        return repository.save(new Campaign(null, name, description), user);
    }

    @Override
    public void deleteCampaign(Long id, String email) {
        if (!repository.userHasCampaign(id, userRepositoryAdapter.findByEmail(email).orElseThrow(UserNotFoundException::new))) {
            throw new InvalidDataException("user does not have this campaign");
        }
        repository.delete(id);
        // Todo: Delete all maps of the campaign
    }
    ...
    public List<Campaign> getAllCampaignsFromUser(String email) {
        return repository.getAllCampaignsFromUser(userRepositoryAdapter.findByEmail(email).orElseThrow(UserNotFoundException::new));
    }
